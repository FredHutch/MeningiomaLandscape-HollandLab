#title: Copy Number Alterations using CaSpER on RNA-Seq
...
cytoband <- read.delim("~/Analysis17_CNV/cytoBand_hg38_ucsc.txt", header = FALSE)
...
centromere <- read.delim("~/Analysis17_CNV/centromere_hg38.txt", header = FALSE)

#control sample ids
controls <- read.csv("~/Analysis17_CNV/CaSpER_test/control.sample.id.csv", header = TRUE)
...
loh <- CaSpER::readBAFExtractOutput(path="~/Analysis17_CNV/CaSpER_Meningioma/compile_menin/", sequencing.type = "bulk", suffix = "baf")
...
CNVcalls <- read.csv("~/Analysis17_CNV/1298_CNVcalls.csv", check.names = FALSE)
CNVcalls <- CNVcalls[, -c(47:48)]
finaldf_CNV = left_join(finaldf, CNVcalls,by="coordinate_ID" )
```
...
```{r}
# Chr 6q
finaldf_CNV$`6q`[is.na(finaldf_CNV$`6q`)] <- "na"
FF <- subset(finaldf_CNV, finaldf_CNV$`6q` == 0) 
FF2 <- subset(finaldf_CNV, finaldf_CNV$`6q` == 1) 
FF3 <- subset(finaldf_CNV, finaldf_CNV$`6q` == -1) 
ggplot(
  finaldf_CNV,
  aes(
    x = UMAP1_2D, 
  ...
  geom_point(size=1.5) +
  geom_point(data=FF, aes(x=UMAP1_2D, y=UMAP2_2D, color=factor(`6q`))) +
  geom_point(data=FF2, aes(x=UMAP1_2D, y=UMAP2_2D, color=factor(`6q`))) +
  geom_point(data=FF3, aes(x=UMAP1_2D, y=UMAP2_2D, color=factor(`6q`))) +
  theme_legend +
  ggtitle("Chr 6q") +
  scale_color_manual(values = c("-1" = "blue",
                                "1" = "red",
                                "0" = "grey", 
                                "na" = "grey"))
ggsave("CNV_6q_leg.pdf", width = 10, height = 7)
ggsave("CNV_6q_noleg.pdf", width = 7, height = 7)
```

Generate UMAPs for each chromosome arm
```{r}
CNVcalls <- read.csv("~/Analysis17_CNV/1298_CNVcalls.csv", check.names = FALSE)
CNVcalls <- CNVcalls[, -c(1,2:33,47:48)]

# Create a PDF file for output
pdf("SupplementalTable_CNAbyChrArm_1298_vst3.pdf")

# Initialize variables
plots_per_page <- 16  # 4x4 grid
total_plots <- ncol(CNVcalls) - 1  # Excluding the first column
...
for (i in seq(1, total_plots, plots_per_page)) {
  ...
  for (z in i:end) {
    ggdf <- cbind(finaldf, cell = CNVcalls[, z + 1])  # Skip first column
    title <- colnames(CNVcalls)[z + 1]
...
Validating CaSpER calls
Expression of NF2 gene in samples that have Chr22 loss/gain according to CaSpER 
```{r}
CF <- finaldf_CNV[, c("coordinate_ID", "22q")]
CF <- subset(CF, !(CF$`22q` == "na"))
NF2_ex <- log2tpm.t[, c("coordinate_ID", "NF2")]
CF2 <- left_join(CF,NF2_ex, by="coordinate_ID" )
CF3 <- CF2[order(CF2$`22q`),]
CF4 <- CF3[, c("coordinate_ID", "NF2")]
metadata5 <- CF3[, c("coordinate_ID", "22q")]
CF5 <- CF4 %>% remove_rownames(.) %>% column_to_rownames(var = "coordinate_ID")
metadata6 <- metadata5 %>% remove_rownames(.) %>% column_to_rownames(var = "coordinate_ID")
pheatmap(CF5, scale = "column", cluster_rows = FALSE, cluster_cols = FALSE, show_rownames = FALSE,
         show_colnames = FALSE, annotation_row = metadata6, color=cols)
ggsave("CNV_22q_NF2expression.pdf", width = 5, height = 5)
```



Kaplan Miere plots on CNA
```{r}
plot_title_size = 20
legend_pt_size = 4
axis_text_size = 15
...
KM plot for samples with and without 10q loss 
```{r}
CNVcalls <- read.csv("~/Analysis17_CNV/1298_CNVcalls.csv", check.names = FALSE)
...
```{r}
CNVcalls <- read.csv("~/Analysis17_CNV/1298_CNVcalls.csv", check.names = FALSE)
chr6qLoss <- subset(CNVcalls, CNVcalls$`6q` == "-1")
...
#select only tumors in cluster A
timerec_A <- subset(timerec, timerec$coordinate_ID %in% cluster_1$coordinate_ID)
