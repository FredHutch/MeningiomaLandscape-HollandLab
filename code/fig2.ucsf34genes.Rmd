...
load gene lists
```{r}
UC <- readxl::read_xlsx("~/Analysis24_UCSFgenepanel/UCSF_genepanel_TableS3.xlsx")
...
```{r}
UCg_rec <- GSVA::gsva(vstcounts, UCg_lst, min.sz=10, max.sz=500, verbose = TRUE)
```
...
UC_log <- vstcounts[rownames(vstcounts) %in% UCgenes,]
...
ggsave("UCSF_34_WHOGRADE_noleg_vst.pdf", width = 7, height = 7)
```


```{r}
ggplot( finaldf_UC %>% 
          arrange(factor(has_the_tumor_reccur)),
        aes(
          x = UMAP1_2D, 
          y = UMAP2_2D,
...
ggsave("UCSF_34_Recurrent_noleg_vst.pdf", width = 7, height = 7)
```
...
ggsave("UCSF_34_Chr22loss_noleg_vst.pdf", width = 7, height = 7)
...
ggsave("UCSF_34_UCSFDNAMethylation_noleg_vst.pdf", width = 7, height = 7)
```
